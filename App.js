...
import { View, Text, StyleSheet, ScrollView, Alert, Modal, TextInput, TouchableOpacity, SafeAreaView, StatusBar, Platform } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import UniversalStorage from './storage';

export default function App() {
  console.log('🚀 PowerTools App iniciando...');
  ...
  const saveData = async () => {
    try {
      await UniversalStorage.multiSet([
  ...
  const loadData = async () => {
    try {
      ...
      const values = await UniversalStorage.multiGet(keys);
  ...
  return (
    <View style={styles.container}>
      {Platform.OS !== 'web' && <StatusBar backgroundColor="#FF6B35" barStyle="light-content" />}
